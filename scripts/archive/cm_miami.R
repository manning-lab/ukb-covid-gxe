cm <- filter(cm, cm[[P_Value_Joint]] > 0, data[[P_Value_Interaction]] > 0)

nlps1 <- -log10(cm[["P_Value_Joint"]])
nlps2 <- -log10(cm[["P_Value_Interaction"]])
...
top <- cbind(select(cm, SNP=RSID, CHR, POS), pvalue=nlps1)[keep1, ]
bottom=cbind(select(cm, SNP=RSID, CHR, POS), pvalue=nlps2)[keep2, ]
topn <- names(top)
