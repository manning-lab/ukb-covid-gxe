...
mdi <- fread("mdi_fuma_v2.gz", data.table=F, stringsAsFactors = F)
  ...
  nlps1 <- -log10(data[[pval_col_1]])
  nlps2 <- -log10(data[[pval_col_2]])
  ...
  newcols <-c(rep(x=c("#D0312D", "#990F02"), length.out=22, each=1),  # Gray/dark gray for alternating chromosomes
  ...
  newcols_bot <-c(rep(x=c("#AAAAAA", "#8A8A8A"), length.out=22, each=1),  # Gray/dark gray for alternating chromosomes
              brewer.pal(3, "Dark2")[c(2, 3, 1)])
  names(newcols_bot) <-c(levels(factor(lims$Color)), "30", "31", "32")
...
#BOTTOM PLOT
  ...
  nlps1 <- -log10(sex[["P_Value_Joint"]])
  nlps2 <- -log10(sex[["P_Value_Interaction"]])
  ...
  top <- cbind(select(sex, SNP=RSID, CHR, POS), pvalue=nlps1)[keep1, ]
  bottom=cbind(select(sex, SNP=RSID, CHR, POS), pvalue=nlps2)[keep2, ]
  topn <- names(top)
  bottomn <- names(bottom)
  top$Location <- "Top"
  bottom$Location <- "Bottom"
